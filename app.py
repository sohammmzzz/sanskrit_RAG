...
Dependencies (pipeline/ must be in the same directory):
  pipeline/parser.py   — LlamaParse v2 agentic_plus multilingual parser
  pipeline/chunker.py  — Smart Sanskrit / Hindi / English multilingual chunker
...
.step-number.active { border-color: var(--gold-dim); color: var(--gold); background: rgba(200,146,42,0.08); }
...
/* Script-type badge on chunk cards */
.script-badge {
  font-family: 'JetBrains Mono', monospace;
  font-size: 0.6rem;
...
.script-bar-row { display: flex; align-items: center; gap: 0.6rem; margin-bottom: 0.4rem; font-family: 'JetBrains Mono', monospace; font-size: 0.68rem; }
.script-bar-label { width: 56px; color: var(--muted); flex-shrink: 0; }
.script-bar-track { flex: 1; height: 3px; background: var(--border); border-radius: 1px; overflow: hidden; }
.script-bar-fill  { height: 100%; border-radius: 1px; }
.script-bar-count { width: 28px; text-align: right; color: var(--text-soft); }
...
# ── Pipeline imports (after page config) ─────────────────────────────────────
...
def _script_breakdown_html(chunks: list[dict]) -> str:
    """Render a mini bar-chart of Sanskrit / Hindi / English / Mixed chunk counts."""
    from collections import Counter
    counts = Counter(c.get("script_type", "mixed") for c in chunks)
    total  = max(len(chunks), 1)
    rows   = ""
    config = [
        ("sanskrit", "var(--gold)",     "Sanskrit"),
        ("hindi",    "var(--hindi-fg)", "Hindi"),
        ("english",  "var(--eng-fg)",   "English"),
        ("mixed",    "var(--muted)",     "Mixed"),
    ]
    for key, colour, label in config:
        n    = counts.get(key, 0)
        pct  = n / total * 100
        rows += f"""
        <div class="script-bar-row">
          <span class="script-bar-label">{label}</span>
          <div class="script-bar-track">
            <div class="script-bar-fill" style="width:{pct:.1f}%;background:{colour};"></div>
          </div>
          <span class="script-bar-count">{n}</span>
        </div>"""
    return f'<div class="script-breakdown">{rows}</div>'
...
with st.sidebar:
    ...
    keys = {
        "LLAMA_CLOUD_API_KEY": "LlamaParse v2",
    ...
    models_info = [
        ("Parser",      "agentic_plus · LlamaParse v2"),
    ...
    st.markdown("""
    <div style="font-family:'JetBrains Mono',monospace;font-size:0.68rem;color:var(--muted);line-height:1.8;">
      ॥ daṇḍa split (Sanskrit)<br>
      । single daṇḍa (Hindi)<br>
      ¶ paragraph / sentence (EN)<br>
      ⌗ sliding window fallback<br>
      ↔ 3-shloka group · 1 overlap
    </div>
    """, unsafe_allow_html=True)
...
with tab_ingest:
    ...
    with col_left:
        ...
        if uploaded_file:
            ...
            <div style="display:flex;align-items:center;gap:0.75rem;margin:0.75rem 0 1rem 0;">
              <div style="width:32px;height:32px;background:var(--card);border:1px solid var(--border);
                          border-radius:1px;display:flex;align-items:center;justify-content:center;font-size:0.8rem;">📄</div>
              <div>
            ...
            if ingest_btn:
                ...
                # ── Step 2: Parse with LlamaParse v2 (agentic_plus) ───────────
                status_box.info("Starting LlamaParse v2 (agentic_plus · multilingual)…")
                try:
                ...
                # ── Step 3: Multilingual chunking ─────────────────────────────
                status_box.info(
                    "Chunking: Sanskrit ॥ daṇḍa → Hindi । daṇḍa → English paragraphs → sliding window…"
                )
                chunks = chunk_document(full_text, source=uploaded_file.name)
                progress_bar.progress(40)

                # Tally by script type for status message
                from collections import Counter
                ...
                with st.expander(f"Preview all {len(chunks)} chunks"):
                    for i, c in enumerate(chunks):
                        ...
                        # shloka_indices is empty for prose chunks — show char count instead
                        meta = (
    ...
    # ── Right: pipeline card + last-run stats ─────────────────────────────────
    with col_right:
        ...
        steps = [
            ...
            ("02", "LlamaParse v2",      "agentic_plus tier · Devanagari custom prompt"),
            ("03", "Multilingual Chunk", "॥ daṇḍa → । daṇḍa → ¶ paragraph → sliding window"),
            ("04", "NIM Embed",          "Dense 1024-dim via NVIDIA NIM baai/bge-m3"),
        ...
        if "last_chunks" in st.session_state:
            ...
            <div class="stat-row">
              ...
              <div class="stat-cell">
                ...
                <div class="stat-value">{len([k for k,v in sc.items() if v > 0])}</div>
              </div>
            </div>
            """, unsafe_allow_html=True)

            # Script breakdown bar chart
            if "last_chunks_data" in st.session_state:
                st.markdown(
                    '<div class="section-label" style="margin-top:1rem;">Script Mix</div>',
                    unsafe_allow_html=True,
                )
                st.markdown(
                    _script_breakdown_html(st.session_state["last_chunks_data"]),
                    unsafe_allow_html=True,
                )
...
with tab_query:
    ...
    with col_q:
        ...
        query = st.text_area(
            "question",
            placeholder=(
                "What does the text say about dharma and duty?\n"
    ...
    if search_btn and query.strip():
        ...
        # ── Step 4: Rerank ────────────────────────────────────────────────────
        with st.status(
            f"Reranking with NVIDIA NIM — keeping top {TOP_K_RERANK}…",
            expanded=False,
        ):
        ...
        if show_chunks:
            st.markdown(
                '<div class="section-label" style="margin-top:1.5rem;">Retrieved Sources</div>',
                unsafe_allow_html=True,
            )
            for i, (result, score) in enumerate(zip(top_results, top_scores), 1):
                ...
                text        = payload["text"]
                script      = payload.get("script_type", "mixed")
                card_cls    = _card_class(script)
        ...
        st.markdown(
            '<div class="section-label" style="margin-top:1.5rem;">Answer</div>',
            unsafe_allow_html=True,
        )
        ...
        if translate_query and sanskrit_query != query:
            st.markdown(
                ...
                f'    <span class="query-detail-value">{query}</span>'
                f'  </div>'
                f'  <div class="query-detail-row">'
                f'    <span class="query-detail-label">sanskrit &nbsp;</span>'
                f'    <span class="query-detail-value">{sanskrit_query}</span>'
                f'  </div>'
                f'</div>',
                unsafe_allow_html=True,
            )
